...
int render_sphere_with_lights()
{
    ...
    sphere.material.color = Ray_Tracer::Color3D::Color3D<float>{1, 0.2, 1};
    // transform_matrix = transform_matrix.translate(1.f, 1.0, .0);
    // sphere.transform(transform_matrix);

    //add light source
    Ray_Tracer::Tuple3D::Tuple3D<float> light_position{-10, 10, -10};
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            if (intersection_points)
            {
                ...
                if (intersection_collection.get_hit())
                {
                    ...
                    Ray_Tracer::Tuple3D::Tuple3D<float> point = ray.position(hit->t);
                    Ray_Tracer::Vec3D::Vec3D<float> normal = hit->object.normalize_at(point);
                    Ray_Tracer::Vec3D::Vec3D<float> eye = -ray.direction;
                    Ray_Tracer::Color3D::Color3D<float> color = Ray_Tracer::Material3D<float>::lighting(hit->object.material, light, point, eye, normal);
...
int main(int argc, char *argv[])
{
    ...
    return render_sphere_with_lights();
}