...
TEST(Ray3D, Create)
{
    Ray_Tracer::Tuple3D<float> origin{1,2,3};
    Ray_Tracer::Vec3D<float> direction{4, 5, 6};
    Ray_Tracer::Ray3D<float> ray{origin, direction};
...
TEST(Ray3D, Position)
{
    Ray_Tracer::Tuple3D<float> origin{2, 3, 4};
    Ray_Tracer::Vec3D<float> direction{1, 0, 0};
    Ray_Tracer::Ray3D<float> ray{origin, direction};

    Ray_Tracer::Tuple3D<float> answer1{2, 3, 4};
    Ray_Tracer::Tuple3D<float> position1 = ray.position(0);

    EXPECT_NEAR(position1.x, answer1.x, 0.0001);
    EXPECT_NEAR(position1.y, answer1.y, 0.0001);
    EXPECT_NEAR(position1.z, answer1.z, 0.0001);

    Ray_Tracer::Tuple3D<float> answer2 {3, 3, 4};
    Ray_Tracer::Tuple3D<float> position2 = ray.position(1);

    EXPECT_NEAR(position2.x, answer2.x, 0.0001);
    EXPECT_NEAR(position2.y, answer2.y, 0.0001);
    EXPECT_NEAR(position2.z, answer2.z, 0.0001);

    Ray_Tracer::Tuple3D<float> answer3 {1, 3, 4};
    Ray_Tracer::Tuple3D<float> position3 = ray.position(-1);

    EXPECT_NEAR(position3.x, answer3.x, 0.0001);
    EXPECT_NEAR(position3.y, answer3.y, 0.0001);
    EXPECT_NEAR(position3.z, answer3.z, 0.0001);

    Ray_Tracer::Tuple3D<float> answer4{4.5, 3, 4};
    Ray_Tracer::Tuple3D<float> position4 = ray.position(2.5);
...
TEST(Ray3D, SphereIntersect1){
    Ray_Tracer::Tuple3D<float> origin{0, 0, -5};
    Ray_Tracer::Vec3D<float> direction{0, 0, 1};
    Ray_Tracer::Ray3D<float> ray{origin, direction};

    std::string uuid = Ray_Tracer::get_uuid();

    Ray_Tracer::Sphere3D<float> sphere{1.0, uuid};
...
{
    Ray_Tracer::Tuple3D<float> origin{0, 1, -5};
    Ray_Tracer::Vec3D<float> direction{0, 0, 1};
    Ray_Tracer::Ray3D<float> ray{origin, direction};
...
{
    Ray_Tracer::Tuple3D<float> origin{0, 2, -5};
    Ray_Tracer::Vec3D<float> direction{0, 0, 1};
    Ray_Tracer::Ray3D<float> ray{origin, direction};
...
{
    Ray_Tracer::Tuple3D<float> origin{0, 0, 0};
    Ray_Tracer::Vec3D<float> direction{0, 0, 1};
    Ray_Tracer::Ray3D<float> ray{origin, direction};
...
TEST(Ray3D, RayTranslation){
    Ray_Tracer::Tuple3D<float> origin{1, 2, 3};
    Ray_Tracer::Vec3D<float> direction{0, 1, 0};
    Ray_Tracer::Ray3D<float> ray{origin, direction};

    Ray_Tracer::Matrix3D<float> mat{};
    Ray_Tracer::Matrix3D<float> translate_matrix = mat.translate(3, 4, 5);
    Ray_Tracer::Ray3D<float> transformed_ray = ray.transform(translate_matrix);
...
TEST(Ray3D, RayScaling)
{
    ...
    Ray_Tracer::Matrix3D<float> mat{};
    Ray_Tracer::Matrix3D<float> translate_matrix = mat.scaling(2, 3, 4);
    Ray_Tracer::Ray3D<float> transformed_ray = ray.transform(translate_matrix);
...
TEST(Ray3D, SphereTransform1)
{
    std::string uuid1 = Ray_Tracer::get_uuid();
    Ray_Tracer::Sphere3D<float> sphere1{1.0, uuid1};
    Ray_Tracer::Matrix3D<float> real_identity_matrix = Ray_Tracer::Matrix3D<float>::get_generic_identity_matrix();
    Ray_Tracer::Matrix3D<float> identity_matrix_from_sphere = sphere1.get_transform();
...
TEST(Ray3D, SphereTransform2)
{
    std::string uuid1 = Ray_Tracer::get_uuid();
    Ray_Tracer::Sphere3D<float> sphere1{1.0, uuid1};
    Ray_Tracer::Matrix3D<float> &transform_matrix = sphere1.transform_matrix;
    Ray_Tracer::Matrix3D<float> translate_matrix = transform_matrix.translate(2, 3, 4);
...
TEST(Ray3D, RaySphereIntersection1)
{
    ...
    Ray_Tracer::Matrix3D<float> &transform_matrix = sphere1.transform_matrix;
    Ray_Tracer::Matrix3D<float> scaling_matrix = transform_matrix.scaling(2, 2, 2);
...
TEST(Ray3D, RaySphereIntersection2)
{
    ...
    Ray_Tracer::Matrix3D<float> &transform_matrix = sphere1.transform_matrix;
    Ray_Tracer::Matrix3D<float> translate_matrix = transform_matrix.translate(5, 0, 0);
    sphere1.transform(translate_matrix);
