...
namespace Ray_Tracer
{
    ...
    public:
            ...
            Ray_Tracer::Vec3D::Vec3D<T> normalv)
        {
            ...
            Ray_Tracer::Color3D::Color3D<T> ambient = effective_color * material.ambient;
            float light_dot_normal = lightv.dot(normalv);
            Ray_Tracer::Color3D::Color3D<T> diffuse{};
            Ray_Tracer::Color3D::Color3D<T> specular{};
            if (light_dot_normal < 0)
            {
                diffuse = Ray_Tracer::Color3D::Color3D<T>{0.0f, 0.0f, 0.0f};
                specular = Ray_Tracer::Color3D::Color3D<T>{0.0f, 0.0f, 0.0f};
            }
            else
            {
            ...
            Ray_Tracer::Color3D::Color3D<float> c = ambient + diffuse + specular;
            return c;
        }
    };
} // namespace Ray_Tracer